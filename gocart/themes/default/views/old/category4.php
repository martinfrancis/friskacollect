<?php include('header.php'); ?>
    ...
    <div class="row">
		...
		<div class="span3">
		<h2>Friska Coffee</h2>
		<?php $cproducts = $this->Product_model->get_products(31); ?>
						<?php if(count($cproducts) > 0):  ?>
							<table class="table menu_items">                         
								<?php foreach($cproducts as $cproduct):?>
								<tr>
									<td>
										<a href="<?php echo site_url(implode('/', $base_url).'/'.$cproduct->slug); ?>"><?php echo $cproduct->name;?></a>
										<?php if($cproduct->excerpt != ''): ?>
										<br /><span class="muted"><small><?php echo $cproduct->excerpt; ?></small></span>
										<?php endif; ?>
									</td>
									<td>
									<?php if((bool)$cproduct->track_stock && $cproduct->quantity < 1) { ?>
												<span class="text-error stock_msg"><strong><?php echo lang('out_of_stock');?></strong></span>
											<?php } ?>
									<span class="pull-right">	  
											<?php if($cproduct->saleprice > 0):?>
												<span class="price-slash muted"><small><?php echo lang('product_reg');?> <?php echo format_currency($cproduct->price); ?></small></span>
												<span class="price-sale"><strong><?php echo lang('product_sale');?> <?php echo format_currency($cproduct->saleprice); ?></strong></span>
											<?php else: ?>
												<span class="price-reg"><?php //echo lang('product_price'); No need for the word Price ?> 
												<?php echo format_currency($cproduct->price); // Currency type set in gocart/config/gocart.php ?></span>
											<?php endif; ?>
									</span>
									</td>	
								</tr>
