...
class Cart extends Front_Controller {
	...
	function category($id)
	{
		...
		//New Grab subcat products
			
		//$data['subcat_products'] = $this->Category_model->get_category_products($data['subcategories']->id, $config['per_page'], $page, $sort_by['by'], $sort_by['sort']);
				
		//$data['subcategories']
		
		//foreach (($data['subcategories']->id) as $q)(
		//$data['subcat_products'] = $this->Category_model->get_category_products($q, $config['per_page'], $page, $sort_by['by'], $sort_by['sort'])
		//)
